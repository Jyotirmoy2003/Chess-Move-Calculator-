...
public sealed class ChessBoardPlacementHandler : MonoBehaviour {
    ...
    internal void Highlight(int row, int col) {
        ...
        Instantiate(_highlightPrefab, tile.transform.position, Quaternion.identity, tile.transform).GetComponent<MovePiece>().Init(row,col);
    }

    //overloading fun Highlight
    internal void Highlight(int row, int col,Color color) {
        ...
        //Instantiate and change the color
        GameObject tempObj=Instantiate(_highlightPrefab, tile.transform.position, Quaternion.identity, tile.transform);
        tempObj.GetComponent<SpriteRenderer>().color=color;
        tempObj.GetComponent<MovePiece>().Init(row,col);
